 ...
 */
class activatepco_model extends CI_Model {
    ...
    private $_table7 = 'tbl_activate_pco';
    private $_table8 = 'master_project';
    ...
    public function data($key = array(), $order_by = '', $direction = 'asc', $limit = NULL, $offset = 0) {
        $this->db->select('d.*,a.*, c.*');
        $this->db->from($this->_table7 . ' d');
        $this->db->join($this->_table1 . ' a', 'a.id_user = d.id_user');
        $this->db->join($this->_table8 . ' c', 'd.id_project = c.id_m_project');
    ...
    public function dataa($key = array(), $order_by = '', $direction = 'asc', $limit = NULL, $offset = 0) {
    ...
    public function create($data) {
        return $this->db->insert($this->_table7, $data);
    }

    public function update($key, $data) {
        return $this->db->update($this->_table7, $data, $this->_key($key, FALSE));
    }

    public function delete($key) {
        return $this->db->delete($this->_table7, $this->_key($key, FALSE));
    }
    ...
    public function options($key = array(), $default = array()) {
      ...
      public function optionsby($key = array(), $default = array()) {
        if (count($default) == 0) {
            $default = array('' => '--Select--');
        }

        $option = $default;
        $data   = $this->dataa($key)->get();

        foreach ($data->result() as $value) {
            $option[$value->id_user] = $value->username_user;
        }

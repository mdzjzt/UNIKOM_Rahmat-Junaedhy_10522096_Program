...
class surat_spbj extends MX_Controller
{
    ...
    {   
        if(hprotection::must_ajax($this->_module)):
            $data = $this->syssetings_model->data(['key_setting' => 'APP_CONFIG', 'name_setting' => 'default_spbj'])->get()->row();

            $data = array(
                            'pageTitle'  => 'Surat SPBJ',
                            '_modul'     => $this->_module,
                            '_title'     => $this->_title, 
    ...
    public function save()
    {
        ...
        ->update(['key_setting' => 'APP_CONFIG', 'name_setting' => 'default_spbj'],['value_setting' => $this->input->post('value_setting')]);

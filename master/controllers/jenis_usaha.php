<?php
	...
	class jenis_usaha extends MX_Controller
	{
		
		private $_class_name = NULL;
		private $_title		 = 'Bidang Usaha';
		...
		public function load()
		{
			if(hprotection::must_ajax($this->_module)):
				...
				if(!empty($this->input->post('keyword'))) {
					$cond["(LOWER(a.bidang) ILIKE '%{$this->input->post('keyword')}%') "] = NULL;
				}

	            $dataCount  = $this->jenis_usaha_model->data($cond)->count_all_results();
	            $dataResult = $this->jenis_usaha_model->data($cond, $orderBy, $direction, $limit, $offset)->get();
		...
		public function add($id = NULL)
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				if(hprotection::must_ajax($this->_module . '/add')):
					...
					$data['bidang']		 = $this->jenis_usaha_model->options([], []);
					...
					if($id):
						...
						$q = $this->jenis_usaha_model->data($id)->get();
        ...
        # Cek data edit
	        if ($this->jenis_usaha_model->data($id)->count_all_results() > 0) {
	            $this->add($id);
	        } else {
		...
		public function proses()
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				...
				if(hprotection::must_ajax($this->_module .'/404')):
					...
					$this->form_validation->set_rules('bidang', '<i class="fa fa-warning"> Nama Bidang</i>', 'trim|required|max_length[200]');
					...
					if($this->form_validation->run($this)):
						...
						if($id):
							if($this->jenis_usaha_model->update($id,$data)):
								$message = ['type' => 'info', 'message' => 'Data Berhasil Diupdate','return' => '_afterSubmit(false)'];
							endif;
						else:
							$data['id'] = $this->jenis_usaha_model->getId();

							if($this->jenis_usaha_model->create($data)):
		...
		public function delete($id = NULL)
		{
			if($this->laccess->otoritas('delete')):
				# CHECK
				$check = $this->jenis_usaha_model->data($id)->count_all_results();

				if($check > 0):
					if($this->jenis_usaha_model->delete($id)):

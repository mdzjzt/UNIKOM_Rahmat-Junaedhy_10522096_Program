<?php
	...
	class bank extends MX_Controller
	{
		...
		public function load()
		{
			if(hprotection::must_ajax($this->_module)):
				...
				if(!empty($this->input->post('keyword'))) {
					$cond["(LOWER(a.code) ILIKE '%{$this->input->post('keyword')}%' OR LOWER(a.name) ILIKE '%{$this->input->post('keyword')}%') "] = NULL;
				}

	            $dataCount  = $this->bank_model->data($cond)->count_all_results();
	            $dataResult = $this->bank_model->data($cond, $orderBy, $direction, $limit, $offset)->get();
			...
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				if(hprotection::must_ajax($this->_module . '/add')):
					...
					if($id):
						...
						$data = $this->bank_model->data($id)->get();
        ...
        # Cek data edit
	        if ($this->bank_model->data($id)->count_all_results() > 0) {
	            $this->add($id);
	        } else {
		...
		public function proses()
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				...
				if(hprotection::must_ajax($this->_module .'/404')):
					...
					#check exist
					$exist_kode = NULL;
					$kode       = trim($this->input->post('code'));

					if(!$id):
						if(!empty($kode)):
						$check_kode = $this->bank_model->is_exist($kode);
							if($check_kode):
								$exist_kode = '|is_exist';
							endif;
						endif;
					...
					$this->form_validation->set_rules('code', '<i class="fa fa-warning"> Kode</i>', 'trim|required|max_length[200]' .$exist_kode);
					$this->form_validation->set_rules('name', '<i class="fa fa-warning"> Nama </i>', 'trim|required|max_length[200]');
		...
		public function delete($id = NULL)
		{
			if($this->laccess->otoritas('delete')):
				# CHECK
				$check = $this->bank_model->data($id)->count_all_results();

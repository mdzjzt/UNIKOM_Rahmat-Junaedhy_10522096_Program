<?php
	...
	class billing extends MX_Controller
	{
		...
		public function load()
		{
			if(hprotection::must_ajax($this->_module)):
				...
				if (!empty($data_cond['keyword'])):
	                $cond["(LOWER(a.billing_kualifikasi) ILIKE '%{$data_cond['keyword']}%' OR LOWER(a.billing_posisi) ILIKE '%{$data_cond['keyword']}%' )"] = NULL;
	            endif;

	            $data_count  = $this->billing_model->data($cond)->count_all_results();
	            $data_result = $this->billing_model->data($cond, 'billing_kualifikasi', 'asc', $limit, $offset)->get();
	            ...
	            foreach ($data_result->result() as $dt):
	            	...
	            	$rows[] = array(
	            				hgenerator::columns_align($no,'center'),
		            				$dt->billing_kualifikasi,
		            				$dt->billing_posisi,
		            				$dt->billing_pengalaman_kerja,
		            				number_format($dt->billing_harga_min),
		            				number_format($dt->billing_harga_max),
			...
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				if(hprotection::must_ajax($this->_module . '/add')):
					...
					if($id):
						...
						$data_edit = $this->billing_model->data($id)->get();
        ...
        # Cek data edit
	        if ($this->billing_model->data($id)->count_all_results() > 0) {
	            $this->add($id);
	        } else {
		...
		public function proses()
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				...
				if(hprotection::must_ajax($this->_module .'/404')):
					...
					$this->form_validation->set_rules('billing_kualifikasi', '<i class="fa fa-warning"> Kualifikasi</i>', 'trim|required');
					$this->form_validation->set_rules('billing_posisi', '<i class="fa fa-warning"> Posisi</i>', 'trim|required');
					$this->form_validation->set_rules('billing_pengalaman_kerja', '<i class="fa fa-warning"> Pengalaman</i>', 'trim|required');
					$this->form_validation->set_rules('billing_harga_min', '<i class="fa fa-warning"> Harga Min</i>', 'trim|required');
					$this->form_validation->set_rules('billing_harga_max', '<i class="fa fa-warning"> Harga Max</i>', 'trim|required');

					if($this->form_validation->run($this)):
						$message = array('error', 'Error!', 'Terjadi error pada saat penyimpanan, silahkan coba lagi.', '');

						$data = array(
										'billing_kualifikasi' => $this->input->post('billing_kualifikasi'),
										'billing_posisi'	  => $this->input->post('billing_posisi'), 
										'billing_pengalaman_kerja' => $this->input->post('billing_pengalaman_kerja'),
										'billing_harga_min'	  => $this->input->post('billing_harga_min'),
										'billing_harga_max'	  => $this->input->post('billing_harga_max'),
									);

						if($id):
							if($this->billing_model->update($id,$data)):
								$message = array(TRUE,'Proses Berhasil','Data Berhasil Disimpan','__after_process(0)');
							endif;
						else:
							if($this->billing_model->create($data)):
		...
		public function delete($id = NULL)
		{
			if($this->laccess->otoritas('delete')):
				# CHECK
				$check = $this->billing_model->data($id)->count_all_results();

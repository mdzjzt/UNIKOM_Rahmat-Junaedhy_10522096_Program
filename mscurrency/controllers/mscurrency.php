...
class mscurrency extends MX_Controller {

    private $_class_name = NULL;
    private $_title = ' MASTER CURRENCY ';
    private $_module = 'mscurrency';
    ...
    function __construct() {
        ...
        $this->load->model('mscurrency_model');
    ...
    public function load()
    {
        if(hprotection::must_ajax($this->_module)) {
            ...
            $dataCount          = $this->mscurrency_model->table($cond)->count_all_results();
            $dataCountFiltered  = $this->mscurrency_model->table($cond)->count_all_results();
            $dataResult         = $this->mscurrency_model->table($cond, $orderBy, $direction, $limit, $offset)->get();
    ...
    public function form($id = NULL){
         if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
            ...
            if ($id) {
                $query = $this->mscurrency_model->table($id)->get()->row();
    ...
    public function save($id = NULL) {
        if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
            if (hprotection::must_ajax($this->_module . '/404')) {
                ...
                $this->form_validation->set_rules('nama_currency', '<i class="fa fa-warning"> Nama Currency</i>', 'trim|required');
                $this->form_validation->set_rules('simbol_currency', '<i class="fa fa-warning"> Simbol currency</i>', 'trim|required');
                $this->form_validation->set_rules('currency', '<i class="fa fa-warning">  Description</i>', 'trim|required');

                if($this->form_validation->run($this)) {

                    $data = [
                        'nama_currency'    => $this->input->post('nama_currency'),
                        'simbol_currency'     => $this->input->post('simbol_currency'),
                    ...
                    if ($id) {
                        ...
                        if ($id = $this->mscurrency_model->update($id, $data)) {
                        ...
                        if ($id = $this->mscurrency_model->create($data)) {
                            $message = ['message' => 'Data Berhasil Disimpan', 'return' => '__afterSubmit()'];
                        }
    ...
    {
        $this->mscurrency_model->delete($id);

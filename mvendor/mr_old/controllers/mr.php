...
class mr extends MX_Controller {

    private $_class_name = NULL;
    private $_title = 'List Project';
    private $_module = 'mr';
    ...
    public function load(){
        if(hprotection::must_ajax($this->_module)) {
            ...
            $cond['status_buat_projek >= 3 '] = NULL;
            ...
            $dataCount          = $this->mr_model->data_project($cond)->count_all_results();
            $dataCountFiltered  = $this->mr_model->data_project($cond)->count_all_results();
            $dataResult         = $this->mr_model->data_project($cond, 'b.nama_project', $direction, $limit, $offset)->get();
            // print_debug($dataResult->result());
            ...
            foreach ($dataResult->result() as $dt) {
                $no++;
                 $action = NULL;
                 // $status="";
                    $dataMR         = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project))->get();
                    $dataMRdraft   = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_save'=>2, 'status_approve'=>0))->get();
                    $dataMRproses   = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_approve'=>0,'status_save'=>1))->get();
                    $dataMRterima   = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_approve'=>1,'status_save'=>1))->get();
                    $dataMRrevisi   = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_approve'=>2,'status_save'=>1))->get();
                    $dataMRrevisi1   = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_approve'=>2,'status_save'=>2))->get();
                    $dataMRtolak    = $this->mr_model->data_mr(array('id_master_project'=>$dt->id_m_project,'status_approve'=>3,'status_save'=>1))->get();
                    ...
                    $total_mr_revisi=$dataMRrevisi->num_rows() + $dataMRrevisi1->num_rows();
                    ...
                    $action .= anchor('#mr/mr/page_list_mr/'.$dt->id_m_project, '<i class="fa fa-eye"></i> Lihat / Buat MR</a>',
                            [
        ...
        public function detail($id){
                ...
                $list = $this->mr_model->data_mr_detail($cond,$order_by,'ASC', $end,$start)->get();
                $list2 = $this->mr_model->data_mr_detail($cond)->get();
                ...
                foreach ($list->result() as $detail) {
                    ...
                    if (empty($detail->no_po) ) {
                        $nama_so = $this->mr_model->data_so($detail->mr_equipment_id)->get();
         ...
         public function detail_info($id){
            // var_dump($_POST['draw']);
            // exit();
                $list = $this->mr_model->data_info_approve($id)->get();
        ...
        public function load_mr($id_project){
            if(hprotection::must_ajax($this->_module)) {
                ...
                $dataCount          = $this->mr_model->data_mr($cond)->count_all_results();
                $dataCountFiltered  = $this->mr_model->data_mr($cond)->count_all_results();
                $dataResult         = $this->mr_model->data_mr($cond, $orderBy, $direction, $limit, $offset)->get();
                ...
                foreach ($dataResult->result() as $dt) {
                    ...
                    if($dt->status_approve == 0 || $dt->status_approve == 2 ){
                        if ($app == 0 ) {
                           ...
                           $action .= anchor('#mr/mr/form_mr/'.$id_project.'/1/'.$dt->mr_id, ' <i class="fa fa-pencil"> </i> </a>',
                                [
                                    'class'          => 'btn btn-info btn-xs margin-right-2',
                                    'rel'            => 'tooltip',
                                    'data-placement' => 'top',
                    ...
                    $action .= anchor($this->_module.'/print_mr/'.$id_project.'/'.$dt->mr_id, ' <i class="fa fa-print"> </i> </a>',
                     ...
                     $datalampiran = $this->mr_model->data_file($dt->mr_id)->get();
        ...
        public function load_list_approve($id_project){
            if(hprotection::must_ajax($this->_module)) {
                ...
                if ($status == 1) {
                        ...
                        $dataCount          = $this->mr_model->data_mr_approve($cond)->count_all_results();
                        $dataCountFiltered  = $this->mr_model->data_mr_approve($cond)->count_all_results();
                        $dataResult         = $this->mr_model->data_mr_approve($cond, $orderBy, $direction, $limit, $offset)->get();
                ...
                foreach ($dataResult->result() as $dt) {
                            ...
                            if ($checkk->num_rows() == 0) {
                             ...
                             if ($floworder == $order) {
                                 ...
                                 if ($checkrev->num_rows() == 0 ) {
                                      $action .= anchor('#mr/mr/form_mr_approve/'.$id_project.'/'.$dt->mr_id, '<i class="fa fa-pencil"></i> FORM APPROVE</a>',
                                        [
                                            'class'          => 'btn btn-warning btn-xs margin-right-2',
                                            'rel'            => 'tooltip',
                                            'data-placement' => 'top',
                    ...
                    }else if ($dt->department == $dt->department_flow) {
        ...
        public function form_mr($id_projcet ,$param, $id = NULL){
            if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
               ...
               if ($ye != $yearsnow  ) {
                ...
                $data_project = $this->mr_model->get_data_project($id_projcet)->get()->row();
                ...
                if ($id) {
                  $query = $this->mr_model->data_edit($id, 'mr_equipment_id')->get();
                  $data['queryrevisi'] = $this->mr_model->data_revisi($id)->get();
                  $query_row = $this->mr_model->table(array('mr_id' => $id, ))->get()->row();
                  $data['lampiran_file_mr'] = $this->mr_model->data_file($id)->get();
                  // var_dump($query_row);
                  // exit();
                  ...
                  if ($param == 2) {
                    ...
                    $data['department'] = $query_row->department;
         ...
         public function form_mr_approve($id_projcet , $id = NULL){
            // if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
                ...
                if ($id) {
                  ...
                  $query_row = $this->mr_model->data_edit($id)->get()->row();
        ...
        public function save_approve($id, $id_project){
         if ($this->laccess->otoritas('approve')) {
                if (hprotection::must_ajax($this->_module . '/404')) {
                    ...
                    $check_kode = $this->mr_model->is_exist_approve($kode, $roleidd, $status_approvee, $apnew);

                    $department = $this->db->get_where('material_requisition', array('mr_id' => $id))->row()->department;

                    $dtmr = $this->db->get_where('material_requisition', array('mr_id' => $id))->row();
                ...
                if ($exist_kode == 1) {
                    ...
                    $cond['or_order'] = '0';
                    $cond['skip_order'] = '0';
                    if ($dtmr->mr_site_project == 2) {
                        $cond["department_flow = '0' or department_flow = '".$department."' "]   = NULL;
                    }
                    ...
                    $condd['role_id'] =  $roleId;
                    if ($dtmr->mr_site_project == 2) {
                         $condd["department_flow = '0' or department_flow = '".$department."' "]   = NULL;
                    }
                    ...
                    // exit();
                    if ($dtmr->mr_site_project == 2) {
                         $data_approve   = $this->workflow_detail_model->data_app($cond,'a.role_id')->get()->num_rows();
                    }else{
                    ...
                    // exit();
                            ...
                            $conddd['skip_order']   = 0;
                            $conddd['or_order']   = 0;
                            if ($dtmr->mr_site_project) {
                                $conddd["department_flow = '0' or department_flow = '".$department."' "]   = NULL;
                            }
                    ...
                    if ($id) {
                          ...
                          // var_dump($step, $order, $status);
                          //   exit();


                        if ($this->mr_model->create_approve($data_app)) {
                            ...
                            if ($this->mr_model->update($id, $data)) {
                                ...
                                if ($this->input->post('status_approve') == 1) {
                                   if ($tahap_step < $data_approve) {
                                    $this->send_email_approve($id_project, $id, $param_id_workflow, $tahap_step,0);
                                    }
                                }
        ...
        public function save($id = NULL) {
            if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
                if (hprotection::must_ajax($this->_module . '/404')) {
                    ...
                    if(!$id):
                        if(!empty($kode)):
                        $check_kode = $this->mr_model->is_exist($kode);
                            if($check_kode):
                                $exist_kode = 1;
                            endif;
                        endif;
                           ...
                           $data = NULL;
                            ...
                            if ($this->input->post('type_transaksi') == 1) {
                                 $data = [
                                    ...
                                    'department' => $this->input->post('deparment') ? $this->input->post('deparment') : 0,
                                 ];
                            }else{
                                $data = [
                                    'id_master_project' => $this->input->post('id_project'),
                            ...
                            if ($id) {
                                ...
                                if($this->input->post('remove_lampiran')) {
                                    foreach ($this->input->post('remove_lampiran') as $key => $value) {
                                        $this->mr_model->deletefilelampiran($this->input->post('remove_lampiran')[$key]);
                                    }
                                }
                            ...
                            if ($this->mr_model->update($id, $data_edit)) {
                                ...
                                if ($this->input->post('type_equipment')){
                                    ...
                                    if ($this->input->post('status_save') == 1) {
                                        $this->send_email_approve($this->input->post('id_project'), $id, $param_id_workflow, $flow_order,$department);
                                        $this->mr_persetujuan_model->delete(['project_id' => $this->input->post('id_project'),'mr_id'=>$id]);
                                        $this->set_workflow($this->input->post('id_project'),$id,$department);
                                ...
                                if($checkFlow->num_rows() > 0) {
                                 ...
                                 if ($id = $this->mr_model->create($data)) {
                                    ...
                                    if ($this->input->post('status_save') != 2) {
                                        $this->send_email_approve($this->input->post('id_project'), $id_mr, $param_id_workflow, $flow_order,$department);
                                        $this->set_workflow($this->input->post('id_project'),$id_mr,$department);
                                    }
        ...
        private function set_workflow($project_id,$mr_id,$department = NULL)
        {
            $cond = [
                        'workflow_id' => $this->_workflow_persetujuan,
    ...
    public function print_mr($id_projcet, $id_mr){
       if ($this->laccess->otoritas('import') || $this->laccess->otoritas('export')) {
        ...
        $condd= [];
        $condd['id_m_project'] = $id_projcet;
        $condd['mr_id'] = $id_mr;
        $dataTable = $this->mr_model->table_equipment($condd)->get();
        // var_dump($dataTable->result());
        // exit();
        $cond= [];
        $cond['id_m_project'] = $id_projcet;
        $cond['mr_id'] = $id_mr;
      
        $dataResult= $this->mr_model->data_result_print($cond)->get();
        $data['table'] = $dataTable->result();
        $data['data'] = $dataResult->row();
        $count_print = $this->mr_model->count_print_mr(array('mr_id' => $id_mr))->get();
        ...
        if($count_print->num_rows() > 0){
            ...
            $this->mr_model->update_count_print_mr($id_mr, $data_edit);
            $result = $this->mr_model->count_print_mr(array('mr_id' => $id_mr))->get()->row();
            $print = $result->print;
        ...
        $this->lpdf->print_standard('A4','P',$print);
   ...
   public function in_range($number, $min, $max)
    ...
    public function detail_info_list_approved($id_project,$id_mr, $param = NULL){
         ...
         $datamr = $this->db->get_where('material_requisition', array('mr_id' => $id_mr))->row();
            ...
            if ( $datamr->department != 0) {
                $list = $this->mr_model->data_info_list_approve_new($id_project, $param, $datamr->department)->get();
             }else{
                $list = $this->mr_model->data_info_list_approve($id_project, $param)->get();
             }
            ...
            // exit();
                ...
                foreach ($list->result() as $detail) {
                    ...
                    if ($detail->workflow_id == 'PRO-MR-01' and $detail->project_id == $id_project) {
    ...
    function send_email_approve_all($id_project,$id_mr, $id_workflow, $floworder,$department){
    ...
    function send_email_approve($id_project,$id_mr, $id_workflow, $floworder,$department){

        $cond['a.project_id']= $id_project;
        $cond['a.workflow_id']= $id_workflow;
        $cond['a.flow_order']= $floworder;
        $cond['a.department_flow']= $department;
    ...
    public function import($id_project, $id = NULL){
                    ...
                    $data_project = $this->mr_model->get_data_project($id_project)->get()->row();

<?php
	...
	class kelompok_jabatan extends MX_Controller
	{
		
		private $_class_name = NULL;
		private $_title		 = 'KELOMPOK JABATAN';
		...
		public function load()
		{
			if(hprotection::must_ajax($this->_module)):
				...
				if (!empty($data_cond['keyword'])):
	                $cond["(LOWER(a.kelompok_jabatan_nama) ILIKE '%{$data_cond['keyword']}%') "] = NULL;
	            endif;

	            $data_count  = $this->kelompok_jabatan_model->data($cond)->count_all_results();
	            $data_result = $this->kelompok_jabatan_model->data($cond, 'kelompok_jabatan_nama', 'asc', $limit, $offset)->get();
			...
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				if(hprotection::must_ajax($this->_module . '/add')):
					...
					if($id):
						...
						$data_edit = $this->kelompok_jabatan_model->data($id)->get();
        ...
        # Cek data edit
	        if ($this->kelompok_jabatan_model->data($id)->count_all_results() > 0) {
	            $this->add($id);
	        } else {
		...
		public function proses()
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')):
				...
				if(hprotection::must_ajax($this->_module .'/404')):
					...
					$nama       = trim($this->input->post('kelompok_jabatan_nama'));

					if(!$id):
						if(!empty($nama)):
							$check_nama = $this->kelompok_jabatan_model->is_exist($nama);
							if($check_nama):
								$exist_nama = '|is_exist';
							endif;
						endif;
					...
					$this->form_validation->set_rules('kelompok_jabatan_nama', '<i class="fa fa-warning"> Jenis Jabatan</i>', 'trim|required|max_length[200]'.$exist_nama);

					if($this->form_validation->run($this)):
						$message = array('error', 'Error!', 'Terjadi error pada saat penyimpanan, silahkan coba lagi.', '');

						$data = array(
										'kelompok_jabatan_nama'	  => $this->input->post('kelompok_jabatan_nama'), 
									);

						if($id):
							if($this->kelompok_jabatan_model->update($id,$data)):
								$message = array(TRUE,'Proses Berhasil','Data Berhasil Disimpan','__after_process(0)');
							endif;
						else:
							if($this->kelompok_jabatan_model->create($data)):
		...
		public function delete($id = NULL)
		{
			if($this->laccess->otoritas('delete')):
				# CHECK
				$check = $this->kelompok_jabatan_model->data($id)->count_all_results();

				if($check > 0):
					if($this->kelompok_jabatan_model->delete($id)):

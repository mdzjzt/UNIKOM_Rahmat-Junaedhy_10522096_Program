<?php
	...
	class unit_kerja extends MX_Controller
	{
		...
		{	
			if(hprotection::must_ajax($this->_module)) {
	            ...
	            $this->log_activity_model->save([
	                        'module'   => $this->_module,
	                        'event'    => 'View',
	                        'activity' => 'View Master Unit Kerja'
	            ]);
		...
		public function load()
		{
			if(hprotection::must_ajax($this->_module)) {
	            ...
	            if (!empty($this->input->post('keyword'))) {
	                $cond["(LOWER(a.unit_kerja_id) ILIKE '%{$this->input->post('keyword')}%' OR LOWER(a.unit_kerja_nama) ILIKE '%{$this->input->post('keyword')}%' )"] = NULL;
	            }

	            $dataCount          = $this->unit_kerja_model->data()->count_all_results();
	            $dataCountFiltered  = $this->unit_kerja_model->data($cond)->count_all_results();
	            $dataResult         = $this->unit_kerja_model->data($cond, $orderBy, $direction, $limit, $offset)->get();
		...
		public function add($id = NULL)
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
				...
				if($id) {
					...
					$data['data']		 = $this->unit_kerja_model->data($id)->get()->row();
				}
		...
		{
	        if ($this->unit_kerja_model->data($id)->count_all_results() > 0) {
	            $this->add($id);
	        } else {
		...
		public function save($id = NULL)
		{
			if($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
				...
				if(hprotection::must_ajax($this->_module .'/404')) {
					...
					if(!$id) {
	                    $unique_kode = '|is_unique[hr_ref_unit_kerja.unit_kerja_id]';
	                } else {
	                    $query = $this->unit_kerja_model->data($id)->get()->row();
	                    if($this->input->post('unit_kerja_id') !== $query->unit_kerja_id) 
	                    	$unique_kode = '|is_unique[hr_ref_unit_kerja.unit_kerja_id]';
	                }

	                $this->form_validation->set_rules('unit_kerja_id', '<i class="fa fa-warning"> Kode</i>', 'trim|required|max_length[200]' .$unique_kode);
	                $this->form_validation->set_rules('unit_kerja_nama', '<i class="fa fa-warning"> Nama Unit Kerja</i>', 'trim|required|max_length[200]');

					if ($this->form_validation->run($this)) {

						$data =[
							'unit_kerja_id'	  => $this->input->post('unit_kerja_id'),
							'unit_kerja_nama'	  => $this->input->post('unit_kerja_nama'), 
							'cabang_id'	  		  => $this->input->post('cabang_id') ? $this->input->post('cabang_id') : NULL,
						];

						if($id) {
							
							if($this->unit_kerja_model->update($id, $data)) {
	                            ...
	                            $this->log_activity_model->save([
	                                        ...
	                                        'sistem'   => TRUE,
	                                        'event'    => 'Insert',
	                                        'activity' => 'Insert Master Unit Kerja'
	                            ]);
							}

						} else {

							if($this->unit_kerja_model->create($data)) {
	                            ...
	                            $this->log_activity_model->save([
	                                        ...
	                                        'sistem'   => TRUE,
	                                        'event'    => 'Update',
	                                        'activity' => 'Update Master Unit Kerja'
	                            ]);
							}
						}
		...
		public function delete($id = NULL)
		{
			if ($this->laccess->otoritas('delete')) {
	            ...
	            if ($this->unit_kerja_model->data($id)->count_all_results() > 0) {
	                if ($this->unit_kerja_model->delete($id)) {
